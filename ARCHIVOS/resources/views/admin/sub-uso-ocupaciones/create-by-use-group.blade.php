							...
							<div class="col-md-12">
								<div class="form-group has-default bmd-form-group">
									...
									" required>
										...
										@foreach ($useGroups as $_useGroup)
											<option value="{{ $_useGroup->id }}"
												@if($useGroup->id === $_useGroup->id) selected @endif
											>{{ $_useGroup->nombre }}</option>
										@endforeach
									</select>
				...
				<div class="card-footer ">
					<div class="row">
						<div class="col-md-12">
							...
							<a href="{{ route('sub-uso-ocupaciones.index-by-use-group', $useGroup->id) }}" class="btn btn-link ml-auto">Volver</a>
						</div>
					</div>
				</div>
			</div>
